...
def check_events(ai_settings, screen, stats, play_button, ship, aliens, bullets):
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            check_keypup_events(event, ship)
...
def check_keypup_events(event, ship):
...
def update_screen(ai_settings, screen, stats, sb, ship, aliens, bullets, play_button):
    ...
    # Draw the play button if inactive
...
def check_fleet_edges(ai_settings, aliens):
    """Respond appropriately if an alien has touched the edge"""
...
def change_fleet_directions(ai_settings, aliens):
    ...
    for alien in aliens.sprites():
        alien.rect.y += 1 * ai_settings.fleet_drop_speed
...
def check_aliens_bottom(ai_settings, stats, screen, ship, aliens, bullets):
    ...
    for alien in aliens.sprites():
        if alien.rect.bottom >= screen_rect.bottom:
            # Treat this same as if the ship got it
            ship_hit(ai_settings, stats, screen, ship, aliens, bullets)
            break
